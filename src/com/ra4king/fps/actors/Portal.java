...
public class Portal implements Actor {
	...
	/**
	 * Without orientation, the portal is by default on the XY plane with the position as the top left corner of the
	 * quad.
	 */
	public Portal(
		OpenGLWorlds worldsManager,
		World parentWorld,
		Vector3 position,
		Vector2 size,
		...
		this.orientation = new Quaternion(orientation).normalize();
	...
	public void transform(Vector3 position, Quaternion orientation) {
		...
		Quaternion diff =
			new Quaternion(this.orientation).mult(new Quaternion(destPortal.getOrientation()).inverse()).normalize();
