...
public class Chunk {
	...
	public static final int TOTAL_BLOCKS =
		Chunk.CHUNK_BLOCK_WIDTH * Chunk.CHUNK_BLOCK_HEIGHT * Chunk.CHUNK_BLOCK_DEPTH;
	...
	public Chunk(ChunkManager manager, int cornerX, int cornerY, int cornerZ) {
		...
		blocks = new Block[CHUNK_BLOCK_WIDTH * CHUNK_BLOCK_HEIGHT * CHUNK_BLOCK_DEPTH];
		...
		for (int i = 0; i < blockCount; i++) {
			...
			blocks[i] = new Block(this, x, y, z, BlockType.AIR);
		}
	}
	...
	public boolean containsBlock(int x, int y, int z) {
		return x >= cornerX && x < cornerX + CHUNK_BLOCK_WIDTH && y >= cornerY && y < cornerY + CHUNK_BLOCK_HEIGHT &&
	...
	public Block[] getNeighbors(Block b) {
		return getNeighbors(
			b.getX() % Chunk.CHUNK_BLOCK_WIDTH,
			b.getY() % Chunk.CHUNK_BLOCK_HEIGHT,
			b.getZ() % Chunk.CHUNK_BLOCK_DEPTH);
	}
	...
	public void set(BlockType blockType, int x, int y, int z) {
		...
		if (block.getX() != x || block.getY() != y || block.getZ() != z) {
			throw new IllegalArgumentException(String.format(
				"Invalid block found at (%d,%d,%d), its coords are (%d,%d,%d)",
				x,
				y,
				z,
				block.getX(),
