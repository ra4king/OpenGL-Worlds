...
public class World implements CameraUpdate {
	...
	public void updateCamera(Camera camera, long deltaTime) {
		...
		if (deltaTimeBuffer >= 1e9 / 120) {
			final float speed =
				(Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) | Keyboard.isKeyDown(Keyboard.KEY_RSHIFT) ? 150 : 20) *
				deltaTimeBuffer / (float)1e9;
			...
			if (Keyboard.isKeyDown(Keyboard.KEY_E)) {
				orientation.set(Utils.angleAxisDeg(-10f * rotSpeed, Vector3.FORWARD).mult(orientation));
			}
		...
		long diff;
		if ((Mouse.isButtonDown(0) || Keyboard.isKeyDown(Keyboard.KEY_C)) &&
		    (diff = System.nanoTime() - bulletCooldown) > (long)5e7) {
			int bulletSpeed = 300;

			bulletManager.addBullet(new Bullet(new Vector3(position).add(inverse.mult3(new Vector3(1f, -1f, 0f),
			                                                                           new Vector3())),
			                                   inverse.mult3(Vector3.FORWARD, new Vector3()).mult(bulletSpeed),
			                                   1,
			                                   30));
			bulletManager.addBullet(new Bullet(new Vector3(position).add(inverse.mult3(new Vector3(-1f, -1f, 0f),
			                                                                           new Vector3())),
		...
		if ((Mouse.isButtonDown(1) || Keyboard.isKeyDown(Keyboard.KEY_V)) &&
		    (diff = System.nanoTime() - blastCoolDown) > (long)3e8) {
			int blastSpeed = 200;

			bulletManager.addBullet(new Bullet(new Vector3(position).add(inverse.mult3(new Vector3(0, -10f, -2f),
			                                                                           new Vector3())),
			                                   inverse.mult3(Vector3.FORWARD, new Vector3()).mult(blastSpeed),
			                                   20,
			                                   200));
