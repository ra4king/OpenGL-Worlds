...
public class Fractal {
	...
	private class CubeRenderer {
		...
		private void render(MatrixStack matrixStack, float angle, int curDepth, FloatBuffer buffer) {
			float radius =
				(float)Math.pow(pow, getDepth() - curDepth + change * timePassed / SPLIT_TIME) / SIZE_CONSTANT;

			matrixStack.getTop().translate(0, radius, 0);
			try {
				buffer.put(PolygonLoader.loadPlane(new Vector2(0.1f, 2 * radius),
				                                   new Vector3(0, 0, 0),
				                                   true,
				                                   false,
				                                   matrixStack.getTop()));
		...
		public void render(MatrixStack matrixStack) {
			matrixStack.getTop()
			           .translate(0, -20, Math.min(-40 * getDepth() - 40 * change * timePassed / SPLIT_TIME, -40));
