		...
		for (Bullet bullet : bullets) {
			...
			if (bullet.isAlive()) {
				...
				if (bullet.isSolid()) {
					...
					float radius = 0.5f * bullet.getSize();
					// How many cubes fit in radius
					int count = (int)Math.ceil(radius / Chunk.SPACING);
					int maxBlocks = count * count * count;

					Block[] blocks = chunkManager.getBlocks(pos, radius);

					if (blocks.length > Math.max(0, maxBlocks * 0.5)) {
						int destroyCount = 0;
...
//					} else if((block = chunkManager.getBlock(pos, 0.5f * bullet.getSize())) != 
//					null && block.getType() != BlockType.AIR) {
...
//						
//						for(int a = 0; a < 2; a++) {
//							temp.add(new Bullet(pos, new Vector3((float)(Math.random() * 2.0 - 1
//							.0), (float)(Math.random() * 2.0 - 1.0), (float)(Math.random() * 2.0 -
//							1.0)).normalize().mult(100), 1, 5, (long)2.5e8, false, new Vector3
//							(1)));
//						}
//					}
				}
