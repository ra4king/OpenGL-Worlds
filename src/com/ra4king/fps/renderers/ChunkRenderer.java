...
public class ChunkRenderer implements ChunkModifiedCallback {
	...
	public static final int CHUNK_DATA_SIZE = Chunk.TOTAL_BLOCKS * Block.SIZE;
		...
		if (block != null) {
			if (!chunk.containsBlock(block)) {
				throw new IllegalArgumentException(String.format(
					"Invalid block: (%d,%d,%d) of type %s. Chunk corner: (%d,%d,%d)",
					block.getX(),
					block.getY(),
					block.getZ(),
	...
	private void testForSurface(Block block) {
		...
		if (block.getX() == chunk.getCornerX()) {
			neighbor = chunk.getChunkManager()
			                .getChunkContaining(chunk.getCornerX() - Chunk.CHUNK_BLOCK_WIDTH,
			                                    chunk.getCornerY(),
			                                    chunk.getCornerZ());
			if (neighbor != null) {
				neighbor.getCallback().chunkModified(null);
			}
			...
			neighbor = chunk.getChunkManager()
			                .getChunkContaining(chunk.getCornerX(),
			                                    chunk.getCornerY() - Chunk.CHUNK_BLOCK_HEIGHT,
			                                    chunk.getCornerZ());
			if (neighbor != null) {
				neighbor.getCallback().chunkModified(null);
			}
			...
			neighbor = chunk.getChunkManager()
			                .getChunkContaining(chunk.getCornerX(),
			                                    chunk.getCornerY(),
			                                    chunk.getCornerZ() - Chunk.CHUNK_BLOCK_DEPTH);
			if (neighbor != null) {
				neighbor.getCallback().chunkModified(null);
			}
	...
	public void update() {
		if (uploadDirtyCount == BUFFER_COUNT) {
			Stopwatch.start("Update ChunkRender buffer");
	...
	public boolean render(DrawElementsIndirectCommand command, FrustumCulling culling, int baseInstance) {
		...
		if (!culling.isRectPrismInsideFrustum(new Vector3(chunk.getCornerX(),
		                                                  chunk.getCornerY(),
		                                                  -chunk.getCornerZ()).mult(Chunk.SPACING)
		                                                                      .sub(halfSpacing,
		                                                                           halfSpacing,
		                                                                           -halfSpacing),
		...
		if (count > 0) {
			final int DATA_SIZE = blockCount * Block.SIZE;
