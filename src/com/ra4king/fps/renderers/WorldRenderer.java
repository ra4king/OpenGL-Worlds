...
public class WorldRenderer {
	...
	private static int loadTexture(String texName) {
		try (InputStream in = Resources.getInputStream("textures/" + texName)) {
			...
			glTexParameteri(GL_TEXTURE_2D,
			                ...
			                glGetInteger(EXTTextureFilterAnisotropic.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT));
	...
	private void setupDeferredFBO() {
		if (deferredFBO != 0) {
			...
			IntBuffer texs = BufferUtils.createIntBuffer(4)
			                            .put(cameraPositionsTexture)
			                            .put(normalsTexture)
			                            .put(texCoordsTexture)
			                            .put(depthTexture);
			texs.flip();
	...
	public void render(Vector4 clipPlane, Portal surroundingPortal, int currentFbo, Camera camera) {
...

		{
			...
			glUniformMatrix3(normalMatrixUniform,
			                 false,
			                 new Matrix3().set4x4(viewMatrix).inverse().transpose().toBuffer());
...

		{
			...
			glUniformMatrix4(deferredProgram.getUniformLocation("projectionMatrix"),
			                 false,
		...
		bulletRenderer.render(new Matrix4().clearToOrtho(-RenderUtils.getWidth() / 2.0f,
